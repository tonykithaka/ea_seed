...
class FarmingRepo {
  Future FetchFarmingTips() async {
    final SharedPreferences prefs = await _prefs;
    final String token = prefs.getString("token");
    final response =
        await http.get("http://68.183.101.224/api/howtos", headers: {
  ...
  Future FetchFarmingTipDetails(String newsId) async {
    final SharedPreferences prefs = await _prefs;
    final String token = prefs.getString("token");
    final response =
        await http.get("http://68.183.101.224/api/howtos/$newsId", headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
      'Authorization': 'Bearer $token',
    });
